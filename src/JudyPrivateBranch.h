...
//
// If the JP points to a linear or bitmap leaf, jp_DcdPop0 contains the
// Population-1 in LSb's and Decode (Dcd) bytes in the MSB's.  (In practice the
...
//
// TBD:  Don't need two structures, except possibly to force jp_Type to highest
// address!
//
// Note:  The jpo_u union is not required by HP-UX or Linux but Win32 because
// the cl.exe compiler otherwise refuses to pack a bitfield (DcdPop0) with
// anything else, even with the -Zp option.  This is pretty ugly, but
// fortunately portable, and it's all hide-able by macros (see below).

typedef struct _JUDY_POINTER_OTHERS	 // JPO.
	{
	    Word_t	_jpo_Addr;	 // first word:  Pjp_t, Word_t, etc.
	    union {
		Word_t  _jpo_DcdPop0:cJU_BITSPERWORD-cJU_BITSPERBYTE;
		uint8_t _jpo_Bytes[sizeof(Word_t)];	// last byte = jp_Type.
	    } jpo_u;
	} jpo_t;
...
//
// _jpi_1Index[] plus _jpi_LIndex[] together hold as many N-byte (1..3-byte
// [1..7-byte]) Indexes as will fit in sizeof(jpi_t) less 1 byte for _jpi_Type
// (that is, 7..1 [15..1] Indexes).
//
// For Judy1, _jpi_1Index[] is used and _jpi_LIndex[] is not used.
// For JudyL, _jpi_LIndex[] is used and _jpi_1Index[] is not used.
//
// Note:  Actually when Pop1 = 1, jpi_t is not used, and the least bytes of the
// single Index are stored in _jpo_DcdPop0, for both Judy1 and JudyL, so for
// JudyL the _jpo_Addr field can hold the target value.
//
// TBD:  Revise this structure to not overload _jpo_DcdPop0 this way?  The
// current arrangement works, it's just confusing.

typedef struct _JUDY_POINTER_IMMED	// JPI.
	{
	    uint8_t _jpi_1Index[sizeof(Word_t)];	// see above.
	    uint8_t _jpi_LIndex[sizeof(Word_t) - 1];	// see above.
	    uint8_t _jpi_Type;			// JP type, 1 of cJ*_JPIMMED*.
	} jpi_t;
...
// alternate data type such as:  A linear branch which is a list of 2..28 JPs,
// or a bitmap branch which contains 8 lists of 0..32 JPs.  JPs reside only in
// branches of a Judy SM.

typedef union _JUDY_POINTER		// JP.
	{
	    jpo_t _jpo;			// other than immediate indexes.
	    jpi_t _jpi;			// immediate indexes.
	} jp_t, *Pjp_t;
...
#define	jp_1Index  _jpi._jpi_1Index	// for storing Indexes in first  word.
#define	jp_LIndex  _jpi._jpi_LIndex	// for storing Indexes in second word.
#define	jp_Addr	   _jpo._jpo_Addr
#define	jp_DcdPop0 _jpo.jpo_u._jpo_DcdPop0
#define	jp_Type	   _jpo.jpo_u._jpo_Bytes[sizeof(Word_t) - 1]
...
//
// Masks for the bytes in the Dcd and Pop0 parts of jp_DcdPop0:
//
// cJU_DCDMASK() consists of a mask that excludes the (LSb) Pop0 bytes and
// also, just to be safe, the top byte of the word, since jp_DcdPop0 is 1 byte
...
// Mask off the high byte from INDEX to it can be compared to DcdPop0:

#define	JU_TRIMTODCDSIZE(INDEX) ((cJU_ALLONES >> cJU_BITSPERBYTE) & (INDEX))

// Get from jp_DcdPop0 the Pop0 for various branch JP Types:
//
// Note:  There are no simple macros for cJU_JAPBRANCH* Types because their
// populations must be added up and don't reside in an already-calculated
// place.

#define	JU_JPBRANCH_POP0(DcdPop0,cPopBytes) \
	((DcdPop0) & cJU_POP0MASK(cPopBytes))


// METHOD FOR DETERMINING IF OBJECTS HAVE ROOM TO GROW:
//
// __JU_GROWCK() is a generic method to determine if an object can grow in
// place, based on whether the next population size (one more) would use the
// same space.

#define __JU_GROWCK(POP1,MAXPOP1,POPTOWORDS) \
	(((POP1) != (MAXPOP1)) && (POPTOWORDS[POP1] == POPTOWORDS[(POP1) + 1]))

#define	JU_BRANCHBJPGROWINPLACE(NumJPs)	\
	__JU_GROWCK(NumJPs, cJU_BITSPERSUBEXPB, __jU_BranchBJPPopToWords)


// DETERMINE IF AN INDEX IS (NOT) IN A JP'S EXPANSE:

#define	JU_DCDNOTMATCHINDEX(Index,DcdPop0,Pop0Bytes) \
	(((Index) ^ (DcdPop0)) & cJU_DCDMASK(Pop0Bytes))


// NUMBER OF JPs IN AN UNCOMPRESSED BRANCH:
//
// An uncompressed branch is simply an array of 256 Judy Pointers (JPs).  It is
// a minimum cacheline fill object.  Define it here before it's first needed.
...
// versions had a larger struct so a linear branch didn't become a bitmap
// branch until the memory consumed was even, but for speed, it's better to
...
typedef struct __JUDY_BRANCH_LINEAR
	{
...
// jbbs_Pjp pointer's value is unused.
//
...
typedef struct __JUDY_BRANCH_BITMAP_SUBEXPANSE
	{
...
typedef struct __JUDY_BRANCH_BITMAP
	{
...
#define	JU_BRANCHJP_NUMJPSTOWORDS(NumJPs) (__jU_BranchBJPPopToWords[NumJPs])
...
typedef struct __JUDY_BRANCH_UNCOMPRESSED
	{
...
//
// Word_t's per various JudyL structures that have constant sizes.
...
// - 1), but there's no way to assert these within an expression.

#define	JU_DIGITATSTATE(Index,cState) \
	 (((Index) >> (((cState) - 1) * cJU_BITSPERBYTE)) & 0xFF)
