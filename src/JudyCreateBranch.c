...
FUNCTION int __JudyCreateBranchL(
...
FUNCTION int __JudyCreateBranchB(
	Pjp_t	Pjp,		// Build JPs from this place
	Pjp_t	PJPs,		// Array of JPs to put into Bitmap branch
	uint8_t Exp[],		// Array of expanses to put into bitmap
	Word_t  ExpCnt,		// Number of above JPs and Expanses
...
// cascade occurs, meaning a leaf overflows.  Either way ExpCnt can't be very
...
// gone very wrong, or else Judy's design criteria have changed, although in
...
//			Get number of JPs in this sub expanse
			...
			PjpRaw = __JudyAllocJBBJP(NumJP, Pjpm);
                        Pjp    = P_JP(PjpRaw);
...
// Free any previous allocations:
			    ...
			    while(CurrSubExp--)
			    {
				NumJP = __JudyCountBitsB(JU_JBB_BITMAP(Pjbb,
								  CurrSubExp));
				if (NumJP)
				{
				    __JudyFreeJBBJP(JU_JBB_PJP(Pjbb,
...
FUNCTION int __JudyCreateBranchU(
	Pjp_t	  Pjp,
	Pvoid_t	  Pjpm)
{
	jp_t	  JPNull;
...
// Allocate memory for a BranchU:
	...
	PjbuRaw = __JudyAllocJBU(Pjpm);
	if (PjbuRaw == (Pjbu_t) NULL) return(-1);
        Pjbu = P_JBU(PjbuRaw);
#endif

//	Clear Null type bits
	JPNull.jp_Addr	  = 0;
	JPNull.jp_DcdPop0 = 0;

//	Set NULL type to Branch_B 2..7 to cJU_JPNULL 1..6
	JPNull.jp_Type	 = Pjp->jp_Type - cJU_JPBRANCH_B2 + cJU_JPNULL1;
...
// Free the subexpanse:
		...
		__JudyFreeJBBJP(JU_JBB_PJP(Pjbb, ii), jj, Pjpm);
