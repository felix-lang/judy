...
static	Word_t __Judy1LCountSM(const Pjp_t Pjp, const Word_t Index,
			       const Pjpm_t Pjpm);

// Each of Judy1 and JudyL get their own private (static) version of this
// function:

static	int __JudyCountLeafB1(const Pjll_t Pjll, const Word_t Pop1,
			      const Word_t Index);
...
Word_t __JudyJPPop1(const Pjp_t Pjp);
...
#endif
	Pcvoid_t  PArray,	// JAP to first branch/leaf in SM.
	...
	PJError_t PJError)	// optional, for returning error info.
{
	...
	jp_t	  fakejp;	// constructed for calling __Judy1LCountSM().
	Pjp_t	  Pjp;		// JP to pass to __Judy1LCountSM().
...
// first.
//
// Note:  Since even cJU_JAPLEAF* types require counting between two Indexes,
// prepare them here for common code below that calls __Judy1LCountSM(), rather
// than handling them even more specially here.
	...
	else if (JAPtype == cJL_JAPLEAF_POPU1)
	{
	    Pjpm	   = & fakejpm;
	    Pjp		   = & fakejp;
	    Pjp->jp_Addr   = P_JLW(PArray);	// first word of leaf.
	    Pjp->jp_Type   = JAPtype;
	    Pjpm->jpm_Pop0 = 0;
	    pop1	   = 1;
	}
...
// If there's no Index at or above Index1, just return C_JERR (early exit):
...
// in case of a real error:
	    ...
	    if ((pop1above1 = __Judy1LCountSM(Pjp, Index1, Pjpm)) == C_JERR)
	    {
...
//
// - Index1's result already includes a count for Index1 and/or Index2 if
...
// point, before calling __Judy1LCountSM() or equivalent?  Possibly a non-issue
// if a top-state pop1 becomes stored with each Judy1 array.  Also, consider
// whether the first call of __Judy1LCountSM() fills the cache, for common tree
...
// Just as for Index1, __Judy1LCountSM() cannot return 0 (locally == C_JERR)
// except in case of a real error:

	    if ((pop1above2 = __Judy1LCountSM(Pjp, Index2, Pjpm)) == C_JERR)
	    {
		JU_COPY_ERRNO(PJError, Pjpm);		// pass through error.
		return(C_JERR);
	    }
...
//
// Given a pointer to a JP (with invalid jp_DcdPop0 at cJU_ROOTSTATE), a known
...
//
// Note:  This function is not told the current state because it's encoded in
// the JP Type.
//
...
FUNCTION static Word_t __Judy1LCountSM(
...
// checking for outliers?  It's because Index is supposed to be valid, hence
// always match any Dcd bits traversed.
//
// Note:  This assertion turns out to be always true for cState = 3 on 32-bit
// and 7 on 64-bit, but it's harmless, probably removed by the compiler.

#define	CHECKDCD(Pjp,cState) \
	assert(! JU_DCDNOTMATCHINDEX(Index, (Pjp)->jp_DcdPop0, cState))
...
// "counts up" instead of down to minimize cache line fills.  However, it's
...
// ROOT-STATE LEAF that starts with a Pop0 word; just count within the leaf:
	...
	case cJU_JAPLEAF:
	{
	    ...
	    offset = __JudySearchLeafW(Pjlw + 1, (Pjpm->jpm_Pop0) + 1, Index);
...
#if (LOW_POP && JUDYL)
// ----------------------------------------------------------------------------
// ROOT-STATE LEAF, pop1 = 1; just count within the leaf:
//
// Assume the known-valid Index is the one in the leaf.
...
#ifndef DEBUG
	    return(1);				// note:  INCLUSIVE of Index.
#else
	    if (Pjlw[0] == Index) return(1);
	    assert(FALSE);
#endif
	}
...
// ----------------------------------------------------------------------------
// ROOT-STATE LEAF, pop1 = 2; just count within the leaf:
//
// Note:  Calling __JudySearchLeafW() would be overkill; just check if the
// first Index in the leaf matches Index.
...
#ifndef DEBUG
	    return((Pjlw[0] == Index) ? 2 : 1);  // note:  INCLUSIVE of Index.
#else
	    if (Pjlw[0] == Index) return(2);
	    if (Pjlw[1] == Index) return(1);
	    assert(FALSE);
#endif
	}
...
BranchL:
	...
	{
	    if ((pop1 = __JudyJPPop1((Pjbl->jbl_jp) + jpnum)) == cJU_ALLONES)
	    {
	...
	pop1 = __Judy1LCountSM((Pjbl->jbl_jp) + jpnum, Index, Pjpm);
...
BranchB:
	{
	    ...
	    Word_t clbelow;	// cache lines below digit's cache line.
	    Word_t clabove;	// cache lines above digit's cache line.
...
// in JPs above Index's JP, or subtracting the pop1's in JPs below Index's JP.
//
...
// Index itself:
		...
		else // (subexp == findsub)
		{
		    ...
		    clfind = CLPERJPS(__JudyCountBitsB(
...
// Note:  It's impossible to get through the following "if" without setting
// jpnum -- see some of the assertions below -- but gcc -Wall doesn't know
...
// entire expanse and there's no point in recursing to lower levels; but this
// should be so rare that it's not worth checking for;
// Judy1Count()/JudyLCount() never even calls the motor for Index == 0 (all
// bytes).


// COUNT UPWARD, subtracting each "below or at" JP's pop1 from the whole
// expanse's pop1:
//
// Note:  If this causes clbelow + 1 cache line fills including JP's cache
// line, that's OK; at worst this is the same as clabove.
		...
		++jbb_upward;
#endif
		pop1above = pop1;		// subtract JP's at/below Index.

// Count JPs for which to accrue pop1's in this subexpanse:
//
// TBD:  If JU_JBB_BITMAP is cJU_FULLBITMAPB, don't bother counting.

		for (subexp = 0; subexp <= findsub; ++subexp)
		{
		    jpcount = __JudyCountBitsB((subexp < findsub) ?
...
// Subtract pop1's from JPs BELOW OR AT Index (digit):
//
// Note:  The pop1 for Index's JP itself is partially added back later at a
// lower state.
//
...
// COUNT DOWNWARD, adding each "above" JP's pop1:
...
// Count JPs for which to accrue pop1's in this subexpanse:
//
// This is more complicated than counting upward because the scan of digit's
// subexpanse must count ALL JPs, to know where to START counting down, and
// ALSO note the offset of digit's JP to know where to STOP counting down.
...
// Count JPs below digit, inclusive:
		    ...
		    if (subexp == findsub)
		    {
			jpcountbf = __JudyCountBitsB(JU_JBB_BITMAP(Pjbb, subexp)
						  & lowermask);
		    }
...
// Add pop1's from JPs ABOVE Index (digit):
...
// Return the net population ABOVE the digit's JP at this state (in this JBB)
// plus the population AT OR ABOVE Index in the SM under the digit's JP:

	    pop1 = __Judy1LCountSM(BMPJP(findsub, jpnum), Index, Pjpm);
	    if (pop1 == C_JERR) return(C_JERR);		// pass error up.
...
// Index's JP, or subtracting the JPs below Index's JP.
//
// COUNT UPWARD, subtracting the pop1 of each JP BELOW OR AT Index, from the
// whole expanse's pop1:

	    if (digit < (cJU_BRANCHUNUMJPS / 2))
	    {
		pop1above = pop1;		// subtract JP's below Index.
#ifdef SMARTMETRICS
		++jbu_upward;
#endif
...
// COUNT DOWNWARD, simply adding the pop1 of each JP ABOVE Index:
	    ...
	    if ((pop1 = __Judy1LCountSM(Pjbu->jbu_jp + digit, Index, Pjpm))
...
// from jp_Addr and the Population from jp_DcdPop0, while immediates use Pjp
...
// the search functions aren't used for anyway:
//
...
#if (JUDYL || ! JU_64BIT)
	case cJU_JPLEAF1:  LEAFLABOVE(__JudySearchLeaf1);
#endif
	case cJU_JPLEAF2:  LEAFLABOVE(__JudySearchLeaf2);
	case cJU_JPLEAF3:  LEAFLABOVE(__JudySearchLeaf3);

#ifdef JU_64BIT
	case cJU_JPLEAF4:  LEAFLABOVE(__JudySearchLeaf4);
	case cJU_JPLEAF5:  LEAFLABOVE(__JudySearchLeaf5);
	case cJU_JPLEAF6:  LEAFLABOVE(__JudySearchLeaf6);
	case cJU_JPLEAF7:  LEAFLABOVE(__JudySearchLeaf7);
#endif
...
// not really a search, but just a count.
	...
	case cJU_JPLEAF_B1:  LEAFB1ABOVE(__JudyCountLeafB1);
...
// IMMEDIATE:
	...
	case cJU_JPIMMED_1_02:  IMMABOVE(__JudySearchLeaf1,  2);
	case cJU_JPIMMED_1_03:  IMMABOVE(__JudySearchLeaf1,  3);
#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_1_04:  IMMABOVE(__JudySearchLeaf1,  4);
	case cJU_JPIMMED_1_05:  IMMABOVE(__JudySearchLeaf1,  5);
	case cJU_JPIMMED_1_06:  IMMABOVE(__JudySearchLeaf1,  6);
	case cJU_JPIMMED_1_07:  IMMABOVE(__JudySearchLeaf1,  7);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_1_08:  IMMABOVE(__JudySearchLeaf1,  8);
	case cJ1_JPIMMED_1_09:  IMMABOVE(__JudySearchLeaf1,  9);
	case cJ1_JPIMMED_1_10:  IMMABOVE(__JudySearchLeaf1, 10);
	case cJ1_JPIMMED_1_11:  IMMABOVE(__JudySearchLeaf1, 11);
	case cJ1_JPIMMED_1_12:  IMMABOVE(__JudySearchLeaf1, 12);
	case cJ1_JPIMMED_1_13:  IMMABOVE(__JudySearchLeaf1, 13);
	case cJ1_JPIMMED_1_14:  IMMABOVE(__JudySearchLeaf1, 14);
	case cJ1_JPIMMED_1_15:  IMMABOVE(__JudySearchLeaf1, 15);
#endif

#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_2_02:  IMMABOVE(__JudySearchLeaf2,  2);
	case cJU_JPIMMED_2_03:  IMMABOVE(__JudySearchLeaf2,  3);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_2_04:  IMMABOVE(__JudySearchLeaf2,  4);
	case cJ1_JPIMMED_2_05:  IMMABOVE(__JudySearchLeaf2,  5);
	case cJ1_JPIMMED_2_06:  IMMABOVE(__JudySearchLeaf2,  6);
	case cJ1_JPIMMED_2_07:  IMMABOVE(__JudySearchLeaf2,  7);
#endif

#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_3_02:  IMMABOVE(__JudySearchLeaf3,  2);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_3_03:  IMMABOVE(__JudySearchLeaf3,  3);
	case cJ1_JPIMMED_3_04:  IMMABOVE(__JudySearchLeaf3,  4);
	case cJ1_JPIMMED_3_05:  IMMABOVE(__JudySearchLeaf3,  5);

	case cJ1_JPIMMED_4_02:  IMMABOVE(__JudySearchLeaf4,  2);
	case cJ1_JPIMMED_4_03:  IMMABOVE(__JudySearchLeaf4,  3);

	case cJ1_JPIMMED_5_02:  IMMABOVE(__JudySearchLeaf5,  2);
	case cJ1_JPIMMED_5_03:  IMMABOVE(__JudySearchLeaf5,  3);

	case cJ1_JPIMMED_6_02:  IMMABOVE(__JudySearchLeaf6,  2);

	case cJ1_JPIMMED_7_02:  IMMABOVE(__JudySearchLeaf7,  2);
#endif
...
//
// This is a private analog of the __JudySearchLeaf*() functions for counting
...
// if Index's bit is not set (which should never happen, so this is DEBUG-mode
// only), return the 1's-complement equivalent (== negative offset minus 1).
//
...
FUNCTION static int __JudyCountLeafB1(
...
// bitmap.
	...
	if (findsub < (cJU_NUMSUBEXPL / 2))
	{
		...
		count += ((JU_JLB_BITMAP(Pjlb, subexp) == cJU_FULLBITMAPL) ?
			  cJU_BITSPERSUBEXPL :
			  __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, subexp)));
	    }

// This count includes findbit, which should be set, resulting in a base-1
// offset:

	    count += __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, findsub)
...
// J U D Y   J P   P O P 1
//
// This function takes any type of JP other than a root-level JP (cJU_JAP* or
...
// Judy1.h version 4.17.)  We think it's cheaper to call a function containing
...
const	Pjp_t Pjp)		// JP to count.
{
	...
	case cJU_JPBRANCH_U2: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,2) + 1);

	case cJU_JPBRANCH_L3:
	case cJU_JPBRANCH_B3:
	case cJU_JPBRANCH_U3: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,3) + 1);

#ifdef JU_64BIT
	case cJU_JPBRANCH_L4:
	case cJU_JPBRANCH_B4:
	case cJU_JPBRANCH_U4: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,4) + 1);

	case cJU_JPBRANCH_L5:
	case cJU_JPBRANCH_B5:
	case cJU_JPBRANCH_U5: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,5) + 1);

	case cJU_JPBRANCH_L6:
	case cJU_JPBRANCH_B6:
	case cJU_JPBRANCH_U6: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,6) + 1);

	case cJU_JPBRANCH_L7:
	case cJU_JPBRANCH_B7:
	case cJU_JPBRANCH_U7: return(JU_JPBRANCH_POP0(Pjp->jp_DcdPop0,7) + 1);
#endif
	...
	case cJU_JPLEAF7:
#endif
	case cJU_JPLEAF_B1:	return(JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1);
