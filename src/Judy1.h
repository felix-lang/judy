...
//				 (970) 898-2027
//
//		     (and Alan Silverstein, ajs@fc.hp.com)
//
...
//   root-level LeafW (Leaf4 [Leaf8]) leaves are also called JAPLEAF.
//
...
// ****************************************************************************
// JUDY1 ARRAY POINTER (JAP) AND JUDY1 POINTER (JP) TYPE FIELDS
// ****************************************************************************
//
...
//
// A pointer (JAP) to a non-empty Judy1 array is at least 8-byte [16-byte]
...
// This feature can be used to determine that a pointer is NOT a Judy1 array
// pointer.
//
...
// JAP TYPE VALUES:
//
// Note:  The JAP values are carefully coordinated with JudyL.h to minimize
// overlap and maximize the chances of detecting a JudyL array pointer passed
// to a Judy1 function, or vice versa.
//
// Note:  It is required that no valid Judy root pointer can consist of all
// 1's, which would be confused with PPJERR.  This means either JAP type 7 must
// not be used, or a pointer of type 7 must never point to an object of size 8
// bytes (which could fall at a maximum memory address).  cJ1_JAPBRANCH
// satisfies this requirement so long as sizeof(jpm_t) > 8.

	cJ1_JAPNULL	  = J1APNULL,	    // = 0x0, must be == 0.
...
//	cJL_JAPLEAF_POPU1 = JLAPLEAF_POPU1,    = 0x5, leafW with Pop1 == 1.
	cJ1_JAPLEAF       = J1APLEAF,	    // = 0x6, leaf1 Pop0 in word 0.
	cJ1_JAPBRANCH     = J1APBRANCH,	    // = 0x7, == Pjpm_t.
...
// calculations between them.
	...
	cJ1_JPBRANCH_L,		// note:  DcdPop0 field not used.
...
// calculations between them.
	...
	cJ1_JPBRANCH_B,		// note:  DcdPop0 field not used.
...
// calculations between them.
	...
	cJ1_JPBRANCH_U,		// note:  DcdPop0 field not used.
...
#define	cJ1_LEAF2_MAXPOP1    (_J1_MAXB / 2)
#define	cJ1_LEAF3_MAXPOP1    (_J1_MAXB / 3)
#define	cJ1_JAPLEAF_MAXPOP1  ((_J1_MAXB - cJU_BYTESPERWORD) / cJU_BYTESPERWORD)
...
#define	cJ1_LEAF2_MAXPOP1    (_J1_MAXB / 2)
#define	cJ1_LEAF3_MAXPOP1    (_J1_MAXB / 3)
#define	cJ1_LEAF4_MAXPOP1    (_J1_MAXB / 4)
#define	cJ1_LEAF5_MAXPOP1    (_J1_MAXB / 5)
#define	cJ1_LEAF6_MAXPOP1    (_J1_MAXB / 6)
#define	cJ1_LEAF7_MAXPOP1    (_J1_MAXB / 7)
#define	cJ1_JAPLEAF_MAXPOP1  ((_J1_MAXB - cJU_BYTESPERWORD) / cJU_BYTESPERWORD)
#endif
...
typedef struct __JUDY1_BITMAP_LEAF
{
...
typedef struct _JUDY1_POPULATION_AND_MEMORY
{
...
/* 7 */	uint8_t	   jpm_Offset;		// offset into leaf from last lookup.
/* 7 */	uint8_t	   jpm_LastPop0;	// population in jpm_LastJP.
...
#ifndef JU_64BIT
extern const uint8_t __j1_Leaf1PopToWords[cJ1_LEAF1_MAXPOP1 + 1];
#endif
extern const uint8_t __j1_Leaf2PopToWords[cJ1_LEAF2_MAXPOP1 + 1];
extern const uint8_t __j1_Leaf3PopToWords[cJ1_LEAF3_MAXPOP1 + 1];
#ifdef JU_64BIT
extern const uint8_t __j1_Leaf4PopToWords[cJ1_LEAF4_MAXPOP1 + 1];
extern const uint8_t __j1_Leaf5PopToWords[cJ1_LEAF5_MAXPOP1 + 1];
extern const uint8_t __j1_Leaf6PopToWords[cJ1_LEAF6_MAXPOP1 + 1];
extern const uint8_t __j1_Leaf7PopToWords[cJ1_LEAF7_MAXPOP1 + 1];
#endif
extern const uint8_t __j1_LeafWPopToWords[cJ1_JAPLEAF_MAXPOP1 + 1];

// Check if increase of population will fit in same leaf:

#ifndef JU_64BIT
#define	J1_LEAF1GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF1_MAXPOP1, __j1_Leaf1PopToWords)
#endif
#define	J1_LEAF2GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF2_MAXPOP1, __j1_Leaf2PopToWords)
#define	J1_LEAF3GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF3_MAXPOP1, __j1_Leaf3PopToWords)
#ifdef JU_64BIT
#define	J1_LEAF4GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF4_MAXPOP1, __j1_Leaf4PopToWords)
#define	J1_LEAF5GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF5_MAXPOP1, __j1_Leaf5PopToWords)
#define	J1_LEAF6GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF6_MAXPOP1, __j1_Leaf6PopToWords)
#define	J1_LEAF7GROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_LEAF7_MAXPOP1, __j1_Leaf7PopToWords)
#endif
#define	J1_LEAFWGROWINPLACE(Pop1) \
	__JU_GROWCK(Pop1, cJ1_JAPLEAF_MAXPOP1, __j1_LeafWPopToWords)

#ifndef JU_64BIT
#define	J1_LEAF1POPTOWORDS(Pop1)  (__j1_Leaf1PopToWords[Pop1])
#endif
#define	J1_LEAF2POPTOWORDS(Pop1)  (__j1_Leaf2PopToWords[Pop1])
#define	J1_LEAF3POPTOWORDS(Pop1)  (__j1_Leaf3PopToWords[Pop1])
#ifdef JU_64BIT
#define	J1_LEAF4POPTOWORDS(Pop1)  (__j1_Leaf4PopToWords[Pop1])
#define	J1_LEAF5POPTOWORDS(Pop1)  (__j1_Leaf5PopToWords[Pop1])
#define	J1_LEAF6POPTOWORDS(Pop1)  (__j1_Leaf6PopToWords[Pop1])
#define	J1_LEAF7POPTOWORDS(Pop1)  (__j1_Leaf7PopToWords[Pop1])
#endif
#define	J1_LEAFWPOPTOWORDS(Pop1)  (__j1_LeafWPopToWords[Pop1])


// FUNCTIONS TO ALLOCATE OBJECTS:

Pj1pm_t	__Judy1AllocJ1PM(void);				// constant size.

Pjbl_t	__Judy1AllocJBL(	  Pj1pm_t);		// constant size.
Pjbb_t	__Judy1AllocJBB(	  Pj1pm_t);		// constant size.
Pjp_t	__Judy1AllocJBBJP(Word_t, Pj1pm_t);
Pjbu_t	__Judy1AllocJBU(	  Pj1pm_t);		// constant size.

#ifndef JU_64BIT
Pjll_t	__Judy1AllocJLL1( Word_t, Pj1pm_t);
#endif
Pjll_t	__Judy1AllocJLL2( Word_t, Pj1pm_t);
Pjll_t	__Judy1AllocJLL3( Word_t, Pj1pm_t);

#ifdef JU_64BIT
Pjll_t	__Judy1AllocJLL4( Word_t, Pj1pm_t);
Pjll_t	__Judy1AllocJLL5( Word_t, Pj1pm_t);
Pjll_t	__Judy1AllocJLL6( Word_t, Pj1pm_t);
Pjll_t	__Judy1AllocJLL7( Word_t, Pj1pm_t);
#endif

Pjlw_t	__Judy1AllocJLW(  Word_t         );		// no Pj1pm needed.
Pj1lb_t	__Judy1AllocJLB1(	  Pj1pm_t);		// constant size.


// FUNCTIONS TO FREE OBJECTS:

void    __Judy1FreeJ1PM( Pj1pm_t,	 Pj1pm_t);	// constant size.

void    __Judy1FreeJBL(  Pjbl_t,	 Pj1pm_t);	// constant size.
void    __Judy1FreeJBB(  Pjbb_t,	 Pj1pm_t);	// constant size.
void    __Judy1FreeJBBJP(Pjp_t,  Word_t, Pj1pm_t);
void    __Judy1FreeJBU(  Pjbu_t,         Pj1pm_t);	// constant size.

#ifndef JU_64BIT
void    __Judy1FreeJLL1( Pjll_t, Word_t, Pj1pm_t);
#endif
void    __Judy1FreeJLL2( Pjll_t, Word_t, Pj1pm_t);
void    __Judy1FreeJLL3( Pjll_t, Word_t, Pj1pm_t);

#ifdef JU_64BIT
void    __Judy1FreeJLL4( Pjll_t, Word_t, Pj1pm_t);
void    __Judy1FreeJLL5( Pjll_t, Word_t, Pj1pm_t);
void    __Judy1FreeJLL6( Pjll_t, Word_t, Pj1pm_t);
void    __Judy1FreeJLL7( Pjll_t, Word_t, Pj1pm_t);
#endif

void    __Judy1FreeJLW(  Pjlw_t, Word_t, Pj1pm_t);
void    __Judy1FreeJLB1( Pj1lb_t,	 Pj1pm_t);	// constant size.
void    __Judy1FreeSM(   Pjp_t,          Pj1pm_t);	// everything below Pjp.
...
static __inline__ int __JudySearchLeaf1(Pjll_t Pjll, Word_t LeafPop1,
					Word_t Index)
{ SEARCHLEAFEVEN(uint8_t,  Pjll, LeafPop1, Index, cJ1_LEAF1_MAXPOP1); }

static __inline__ int __JudySearchLeaf2(Pjll_t Pjll, Word_t LeafPop1,
					Word_t Index)
{ SEARCHLEAFEVEN(uint16_t, Pjll, LeafPop1, Index, cJ1_LEAF2_MAXPOP1); }

#ifdef JU_64BIT
static __inline__ int __JudySearchLeaf4(Pjll_t Pjll, Word_t LeafPop1,
					Word_t Index)
{ SEARCHLEAFEVEN(uint32_t, Pjll, LeafPop1, Index, cJ1_LEAF4_MAXPOP1); }
#endif

static __inline__ int __JudySearchLeafW(Pjlw_t Pjlw, Word_t LeafPop1,
					Word_t Index)
{ SEARCHLEAFEVEN(Word_t,   Pjlw, LeafPop1, Index, cJ1_JAPLEAF_MAXPOP1); }
...
// "Odd" LeafL types are the same for all platforms:
