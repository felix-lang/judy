...
FUNCTION int __JudyInsertBranch(
...
//	digit for BranchLevel is the LSB:
	...
	XorExp = ((Index ^ Pjp->jp_DcdPop0) & (cJU_ALLONES >> cJU_BITSPERBYTE))
...
//	the new Index to insert:
	...
	DecodeByteO = JU_DIGITATSTATE(Pjp->jp_DcdPop0, BranchLevel);
...
//
//	Note: Pjp->jp_Addr is set by __JudyCreateBranchL()

	Ret = __JudyCreateBranchL(Pjp, JP2, Exp2, 2, Pjpm);
	if (Ret == -1) return(-1);
...
//	Build JP and set type below to: cJU_JPIMMED_X_01
	PjpNull->jp_Addr    = 0;
	PjpNull->jp_DcdPop0 = Index;
	PjpNull->jp_Type    = cJU_JPIMMED_1_01 - 2 + BranchLevel;
...
//	BranchLevel; then use this mask to zero the bits in jp_DcdPop0:

	DCDMask		^= cJU_DCDMASK(BranchLevel);
	Pjp->jp_DcdPop0	&= ~DCDMask;
