...
// leaves (LeafB1's) to Leaf1's, and some types of branches to smaller branches
...
FUNCTION static void __JudyCopy2to3(
...
FUNCTION static void __JudyCopy3to4(
...
FUNCTION static void __JudyCopy4to5(
...
FUNCTION static void __JudyCopy5to6(
...
FUNCTION static void __JudyCopy6to7(
	uint8_t * PDest,	// to where to copy 6-byte Indexes.
	uint8_t * PSrc,		// from where to copy 5-byte indexes.
	Word_t    Pop1,		// number of Indexes to copy.
	Word_t    MSByte)	// most-significant byte, prefix to each Index.
...
FUNCTION int __JudyBranchBToBranchL(
	...
	Pvoid_t	Pjpm)		// for global accounting.
{
	...
	assert(Pjp->jp_Type >= cJU_JPBRANCH_B2);
	assert(Pjp->jp_Type <= cJU_JPBRANCH_B);
...
// Allocate and populate the BranchL:
	...
	if ((PjblRaw = __JudyAllocJBL(Pjpm)) == (Pjbl_t) NULL) return(-1);
	Pjbl = P_JBL(PjblRaw);
...
// Copy JPs from each BranchB subexpanse subarray:
	...
	for (SubExp = 0; SubExp < cJU_NUMSUBEXPB; ++SubExp)
	{
	    ...
	    Pjpjbl += NumJPs;
	    __JudyFreeJBBJP(PjpRaw, NumJPs, Pjpm);	// subarray.
	}
	__JudyFreeJBB(PjbbRaw, Pjpm);		// BranchB itself.
...
FUNCTION int __JudyBranchUToBranchB(
...
FUNCTION int __JudyLeafB1ToLeaf1(
	...
	Pvoid_t	  Pjpm)		// for global accounting.
{
	...
	assert(Pjp->jp_Type == cJU_JPLEAF_B1);
	assert(((Pjp->jp_DcdPop0 & 0xFF) + 1) == cJU_LEAF1_MAXPOP1);

// Allocate JPLEAF1 and prepare pointers:

	if ((PjllRaw = __JudyAllocJLL1(cJU_LEAF1_MAXPOP1, Pjpm)) == 0)
	    return(-1);
...
// Copy all old-LeafB1 value areas from value subarrays to new Leaf1:
	...
	for (SubExp = 0; SubExp < cJU_NUMSUBEXPL; ++SubExp)
	{
	    ...
	    Pop1 = __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, SubExp));  // subarray.
	    assert(Pop1);
...
//
// Note:  jp_DcdPop0 does not change here.
...
FUNCTION Word_t  __JudyLeaf1ToLeaf2(
...
// JPLEAF_B1:
  ...
  JUDYLCODE(Word_t SubExp;)	// in LeafB1.
	    ...
	    Pop1 = JU_JPBRANCH_POP0(Pjp->jp_DcdPop0, 1) + 1; assert(Pop1);
...
// Copy all old-LeafB1 value areas from value subarrays to new Leaf2:
		...
		SubExpPop1 = __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, SubExp));
		assert(SubExpPop1);

		JU_COPYMEM(Pjv2, Pjv1, SubExpPop1);	// copy value areas.
		__JudyLFreeJV(Pjv1Raw, SubExpPop1, Pjpm);
		Pjv2 += SubExpPop1;			// advance through new.
	    }
#endif // JUDYL

	    __JudyFreeJLB1((Pjlb_t) (Pjp->jp_Addr), Pjpm);  // LeafB1 itself.
	    return(Pop1);
...
// JPIMMED_1_01:
//
// Note:  jp_DcdPop0 has 3 [7] bytes of Index (all but most significant byte),
// so the assignment to PLeaf2[] truncates and MSByte is not needed.

	case cJU_JPIMMED_1_01:
	{
	    PLeaf2[0] = Pjp->jp_DcdPop0;	// see above.
...
#endif
	{
	    Pop1 = Pjp->jp_Type - cJU_JPIMMED_1_02 + 2; assert(Pop1);
	    ...
	    JUDYLCODE(__JudyLFreeJV(Pjv1Raw, Pop1, Pjpm);)
	    return(Pop1);
	}
...
FUNCTION Word_t  __JudyLeaf2ToLeaf3(
...
// JPLEAF2:
	...
	case cJU_JPLEAF2:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1; assert(Pop1);
	    __JudyCopy2to3(PLeaf3, PLeaf2, Pop1, MSByte);
#ifdef JUDYL
...
// needed.
	...
	{
	    JU_COPY3_LONG_TO_PINDEX(PLeaf3, Pjp->jp_DcdPop0);	// see above.
...
// JPIMMED_2_0[2+]:
...
#if (JUDY1 || JU_64BIT)
	{
	    ...
	    Pop1 = Pjp->jp_Type - cJU_JPIMMED_2_02 + 2; assert(Pop1);
	    __JudyCopy2to3(PLeaf3, PLeaf2, Pop1, MSByte);
#ifdef JUDYL
	    Pjv2Raw = (Pjv_t) (Pjp->jp_Addr);
	    Pjv2    = P_JV(Pjv2Raw);
	    JU_COPYMEM(Pjv3, Pjv2, Pop1);
	    __JudyLFreeJV(Pjv2Raw, Pop1, Pjpm);
#endif
	    return(Pop1);
	}
...
FUNCTION Word_t  __JudyLeaf3ToLeaf4(
...
// JPLEAF3:
	...
	case cJU_JPLEAF3:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1; assert(Pop1);
	    __JudyCopy3to4(PLeaf4, (uint8_t *) PLeaf3, Pop1, MSByte);
#ifdef JUDYL
...
// JPIMMED_3_01:
//
// Note:  jp_DcdPop0 has 7 bytes of Index (all but most significant byte), so
// the assignment to PLeaf4[] truncates and MSByte is not needed.

	case cJU_JPIMMED_3_01:
	{
	    PLeaf4[0] = Pjp->jp_DcdPop0;	// see above.
...
// JPIMMED_3_0[2+]:
...
#endif
	{
	    ...
	    JUDY1CODE(Pop1 = Pjp->jp_Type - cJU_JPIMMED_3_02 + 2;)
	    JUDYLCODE(Pop1 = 2;)

	    __JudyCopy3to4(PLeaf4, PLeaf3, Pop1, MSByte);
#ifdef JUDYL
	    Pjv3Raw = (Pjv_t) (Pjp->jp_Addr);
	    Pjv3    = P_JV(Pjv3Raw);
	    JU_COPYMEM(Pjv4, Pjv3, Pop1);
	    __JudyLFreeJV(Pjv3Raw, Pop1, Pjpm);
#endif
	    return(Pop1);
	}
...
// Note:  In all following __JudyLeaf*ToLeaf*() functions, JPIMMED_*_0[2+]
...
FUNCTION Word_t  __JudyLeaf4ToLeaf5(
...
// JPLEAF4:
	...
	case cJU_JPLEAF4:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1; assert(Pop1);
	    __JudyCopy4to5(PLeaf5, PLeaf4, Pop1, MSByte);
#ifdef JUDYL
...
// the assignment to PLeaf5[] truncates and MSByte is not needed.
	...
	{
	    JU_COPY5_LONG_TO_PINDEX(PLeaf5, Pjp->jp_DcdPop0);	// see above.
...
// JPIMMED_4_0[4+]:
	...
	case cJ1_JPIMMED_4_03:
	{
	    ...
	    Pop1 = Pjp->jp_Type - cJ1_JPIMMED_4_02 + 2;
	    __JudyCopy4to5(PLeaf5, PLeaf4, Pop1, MSByte);
	    return(Pop1);
	}
#endif // JUDY1
...
FUNCTION Word_t  __JudyLeaf5ToLeaf6(
...
// JPLEAF5:
	...
	case cJU_JPLEAF5:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1; assert(Pop1);
	    __JudyCopy5to6(PLeaf6, PLeaf5, Pop1, MSByte);
#ifdef JUDYL
...
// the assignment to PLeaf6[] truncates and MSByte is not needed.
	...
	{
	    JU_COPY6_LONG_TO_PINDEX(PLeaf6, Pjp->jp_DcdPop0);	// see above.
...
FUNCTION Word_t  __JudyLeaf6ToLeaf7(
...
// JPLEAF6:
	...
	case cJU_JPLEAF6:
	{
	    ...
	    __JudyCopy6to7(PLeaf7, PLeaf6, Pop1, MSByte);
#ifdef JUDYL
...
// the "assignment" to PLeaf7[] is exact and MSByte is not needed.
	...
	{
	    JU_COPY7_LONG_TO_PINDEX(PLeaf7, Pjp->jp_DcdPop0);	// see above.
...
// JPIMMED_6_02:
	...
	case cJ1_JPIMMED_6_02:
	{
	    ...
	    __JudyCopy6to7(PLeaf7, PLeaf6, /* Pop1 = */ 2, MSByte);
	    return(2);
	}
#endif // JUDY1
...
FUNCTION Word_t  __JudyLeaf3ToLeafW(
...
// JPLEAF3:
	...
	case cJU_JPLEAF3:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1;
	    __JudyCopy3toW((PWord_t) Pjlw, PLeaf3, Pop1, MSByte);
#ifdef JUDYL
	    Pjv3 = JL_LEAF3VALUEAREA(PLeaf3, Pop1);
...
// JPIMMED_3_01:
//
// Note:  jp_DcdPop0 has 3 bytes of Index (all but most significant byte), and
// MSByte must be or'd in.

	case cJU_JPIMMED_3_01:
	{
	    Pjlw[0] = MSByte | Pjp->jp_DcdPop0;		// see above.
...
// JPIMMED_3_02:
	...
	case cJU_JPIMMED_3_02:
	{
	    ...
	    __JudyCopy3toW((PWord_t) Pjlw, PLeaf3, /* Pop1 = */ 2, MSByte);
	    return(2);
	}
#endif // JUDY1
...
FUNCTION Word_t  __JudyLeaf7ToLeafW(
	Pjlw_t	Pjlw,		// destination Index part of leaf.
#ifdef JUDYL
	Pjv_t	PjvW,		// destination value part of leaf.
#endif
...
// JPLEAF7:
	...
	case cJU_JPLEAF7:
	{
	    ...
	    Pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1;
	    __JudyCopy7toW((PWord_t) Pjlw, PLeaf7, Pop1, MSByte);
#ifdef JUDYL
...
// JPIMMED_7_01:
//
// Note:  jp_DcdPop0 has 7 bytes of Index (all but most significant byte), and
// MSByte must be or'd in.
...
// JPIMMED_7_02:
	...
	case cJ1_JPIMMED_7_02:
	{
	    ...
	    __JudyCopy7toW((PWord_t) Pjlw, PLeaf7, /* Pop1 = */ 2, MSByte);
	    return(2);
	}
#endif
