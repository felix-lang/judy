...
// caller's *PIndex-1, starting with the JAP (PArray), through branches, if
...
// go to its parent branch in turn.  Upon reaching the JAP, return failure, "no
...
// digits are stored in jp_DcdPop0), compare the relevant digits to the same
// digits in *PIndex-1.  If they are EQUAL, proceed in SM1Get as before.  If
// jp_DcdPop0's digits are GREATER, treat the JP as a dead end and proceed in
// SM2Backtrack.  If jp_DcdPop0's digits are LESS, treat the JP as if it had
// just been found during a backtrack and proceed directly in SM3Findlimit.
//
// Note that Decode bytes can be ignored in SM3Findlimit; they don't matter.
// Also note that in practice the Decode bytes are routinely compared with
// *PIndex-1 because that's simpler and no slower than first testing for
// narrowness.
//
...
// of *PIndex is derived (saved) based on which JP is used in a JAP branch.
// The remaining digits are obtained from the jp_DcdPop0 field in the JP (if
// any) above the immediate or leaf containing the found (prev) Index, plus the
// remaining digit(s) in the immediate or leaf itself.  In the case of a JAP
// leaf, the Index to return is found directly in the leaf.
//
...
// it's a little more efficient for the SM1Get leaf cases to shortcut this and
...
// by a backtrack; it's pretty cheap to set up initially; and it means the
...
//
//	(((Pjp->jp_DcdPop0 ^ cJU_ALLONES) & cJU_POP0MASK(cLevel)) == 0)
...
#endif
	int	  offset;	// linear branch/leaf, from __JudySearchLeaf*().
...
// subexpanse's JP list (to avoid bit-counting again), plus for Judy*Next(),
...
#define	HISTNUMMAX cJU_ROOTSTATE	// maximum branches traversable.
	Pjp_t	APjphist[HISTNUMMAX];	// list of branch JP's traversed.
...
// underflowing *hist[] simply means there's no more room to backtrack =>
...
// SEARCH A BITMAP:
//
// This is a weak analog of __JudySearchLeaf*() for bitmaps.  Return the actual
...
// next-right JP.
//
// Shortcut and skip calling __JudyCountBits*() if the bitmap is full, in which
...
// That is, +1 from current value?  Maybe not, if Digit's bit IS set, +1 would
// be wrong.
...
#define	SEARCHBITMAPL(Bitmap,Digit,Bitposmask)				\
	(((Bitmap) == cJU_FULLBITMAPL) ? (Digit % cJU_BITSPERSUBEXPL) :	\
	 __JudyCountBitsL((Bitmap) & JU_MASKLOWERINC(Bitposmask)) - 1)
...
#define	CHECKDCD(cState)						\
	if (JU_DCDNOTMATCHINDEX(*PIndex, Pjp->jp_DcdPop0, cState))	\
	{								\
	    if ((*PIndex		& cJU_DCDMASK(cState))		\
	      __CDcmp((Pjp->jp_DcdPop0) & cJU_DCDMASK(cState)))		\
	    {								\
		goto SM2Backtrack;					\
	    }								\
	    goto SM3Findlimit;						\
...
// PREPARE TO HANDLE A JAP OR JP BRANCH IN SM1:
//
...
// PREPARE TO HANDLE A JAP OR JP BRANCH IN SM3:
//
...
// HANDLE JAP:
//
// Before even entering SM1Get, check the JAP type.  For JAP branches, traverse
// the JPM; handle JAP leaves directly; but look for the most common cases
// first.
...
// below where it would belong.
	...
	if (JAPtype == cJU_JAPLEAF)
	{
	    ...
	    if ((offset = __JudySearchLeafW(Pjlw + 1, pop1, *PIndex))
		>= 0)				// Index is present.
	    {
		assert(offset < pop1);			  // in expected range.
		JU_RET_FOUND_JAPLEAF(Pjlw, pop1, offset); // *PIndex is set.
	    }
...
#endif
	    JU_RET_FOUND_JAPLEAF(Pjlw, pop1, offset);	// base 0.

	} // if cJU_JAPLEAF

#if (LOW_POP && JUDYL)

// ROOT-STATE LEAF, pop1 = 1; just look within the leaf:
//
// If the valid Index in the leaf is below/above or at *PIndex, return it, else
// "no previous/next".

#define RFP1  return((PPvoid_t) ((Pjlw) + 1))	// for brevity below.
...
#ifdef JUDYPREV
#ifdef notdef
	    if (*PIndex >  Pjlw[0]) { *PIndex = Pjlw[0]; RFP1; }
	    if (*PIndex == Pjlw[0])		         RFP1;
#else // Doug says this is actually faster:
	    if (*PIndex >= Pjlw[0]) { *PIndex = Pjlw[0]; RFP1; }
#endif // notdef
#else // JUDYNEXT
#ifdef notdef
...
#endif // JUDYNEXT
							 JU_RET_NOTFOUND;
	} // if cJL_JAPLEAF_POPU1


// ROOT-STATE LEAF, pop1 = 2; just look within the leaf:
//
// Return the higher/lower of the 2 valid Indexes in the leaf that is
// below/above or at *PIndex, if any, else "no previous/next".

	if (JAPtype == cJL_JAPLEAF_POPU2)
	{
	    Pjlw_t Pjlw = P_JLW(PArray);	// first word of leaf.

#define RFP2(Offset) return((PPvoid_t) ((Pjlw) + (Offset) + 2))  // for brevity.

#ifdef JUDYPREV
#ifdef notdef
	    if (*PIndex >  Pjlw[1]) { *PIndex = Pjlw[1]; RFP2(1); }
	    if (*PIndex == Pjlw[1])		         RFP2(1);
	    if (*PIndex >  Pjlw[0]) { *PIndex = Pjlw[0]; RFP2(0); }
	    if (*PIndex == Pjlw[0])		         RFP2(0);
#else // Doug says this is actually faster:
	    if (*PIndex >= Pjlw[1]) { *PIndex = Pjlw[1]; RFP2(1); }
	    if (*PIndex >= Pjlw[0]) { *PIndex = Pjlw[0]; RFP2(0); }
#endif // notdef
...
// new JP, so it's easy to do constant shifts/masks, although this requires
...
// JP's offset, and iterate to the next JP:

	    if ((offset = __JudySearchLeaf1((Pjll_t) (Pjbl->jbl_Expanse),
...
//
// Get the ideal location of digit's JP, and if there's no next-left/right JP
...
// There's a next-left/right JP in the current BranchL; save its digit in
...
// Locate the digit's JP in the subexpanse list, if present, otherwise the
...
//
// Record the parent JP and the next JP's offset; and iterate to the next JP.
...
//
// If there's a next-left/right JP in the current BranchB, shortcut to
...
// Save the next-left/right JP's digit in *PIndex:
...
// There's no next-left/right JP in the BranchB:
//
...
// Found JP matching current digit in *PIndex:
//
// Record the parent JP and the next JP's digit, and iterate to the next JP.
//
// TBD:  Instead of this, just goto SM1Get, and add cJU_JPNULL* cases to the
// SM1Get state machine?  Then backtrack?  However, it means you can't detect
// an inappropriate cJU_JPNULL*, when it occurs in other than a BranchU, and
// return JU_RET_CORRUPT.

	    if (! JPNULL(Pjp2->jp_Type))	// digit has a JP.
	    {
		HISTPUSH(Pjp, digit);
		Pjp = Pjp2;
		goto SM1Get;
...
// save its digit in *PIndex and shortcut to SM3Findlimit:
		...
		Pjp = (Pjbu->jbu_jp) + (++digit);
#endif
		if (JPNULL(Pjp->jp_Type)) continue;

		JU_SETDIGIT(*PIndex, digit, state);
		goto SM3Findlimit;
	    }

// There's no next-left/right JP in the BranchU:
//
// Shortcut and start backtracking one level up; ignore the current Pjp because
...
#if (JUDYL || ! JU_64BIT)
	case cJU_JPLEAF1:  CHECKDCD(1); SM1LEAFL(__JudySearchLeaf1);
#endif
	case cJU_JPLEAF2:  CHECKDCD(2); SM1LEAFL(__JudySearchLeaf2);
	case cJU_JPLEAF3:  CHECKDCD(3); SM1LEAFL(__JudySearchLeaf3);

#ifdef JU_64BIT
	case cJU_JPLEAF4:  CHECKDCD(4); SM1LEAFL(__JudySearchLeaf4);
	case cJU_JPLEAF5:  CHECKDCD(5); SM1LEAFL(__JudySearchLeaf5);
	case cJU_JPLEAF6:  CHECKDCD(6); SM1LEAFL(__JudySearchLeaf6);
	case cJU_JPLEAF7:  CHECKDCD(7); SM1LEAFL(__JudySearchLeaf7);
#endif
...
//
// Get the ideal location of Index, and if there's no next-left/right Index in
...
// There's a next-left/right Index in the current LeafL / Immed; shortcut by
...
//
// If there's a next-left/right Index in the current LeafB1, which for
...
// Save the next-left/right Indexes's digit in *PIndex:
...
// There's no next-left/right Index in the LeafB1:
//
// Shortcut and start backtracking one level up; ignore the current Pjp because
...
#define	SM1IMM_01							  \
	SM1IMM_01_SETPOP1;						  \
	offset = (((Pjp->jp_DcdPop0) <  JU_TRIMTODCDSIZE(*PIndex)) ? ~1 : \
		  ((Pjp->jp_DcdPop0) == JU_TRIMTODCDSIZE(*PIndex)) ?  0 : \
								     ~0); \
	goto SM1LeafLImm
...
// and in Judy*Count() also.
	...
	case cJU_JPIMMED_1_02:  SM1IMM(__JudySearchLeaf1,  2);
	case cJU_JPIMMED_1_03:  SM1IMM(__JudySearchLeaf1,  3);
#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_1_04:  SM1IMM(__JudySearchLeaf1,  4);
	case cJU_JPIMMED_1_05:  SM1IMM(__JudySearchLeaf1,  5);
	case cJU_JPIMMED_1_06:  SM1IMM(__JudySearchLeaf1,  6);
	case cJU_JPIMMED_1_07:  SM1IMM(__JudySearchLeaf1,  7);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_1_08:  SM1IMM(__JudySearchLeaf1,  8);
	case cJ1_JPIMMED_1_09:  SM1IMM(__JudySearchLeaf1,  9);
	case cJ1_JPIMMED_1_10:  SM1IMM(__JudySearchLeaf1, 10);
	case cJ1_JPIMMED_1_11:  SM1IMM(__JudySearchLeaf1, 11);
	case cJ1_JPIMMED_1_12:  SM1IMM(__JudySearchLeaf1, 12);
	case cJ1_JPIMMED_1_13:  SM1IMM(__JudySearchLeaf1, 13);
	case cJ1_JPIMMED_1_14:  SM1IMM(__JudySearchLeaf1, 14);
	case cJ1_JPIMMED_1_15:  SM1IMM(__JudySearchLeaf1, 15);
#endif

#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_2_02:  SM1IMM(__JudySearchLeaf2,  2);
	case cJU_JPIMMED_2_03:  SM1IMM(__JudySearchLeaf2,  3);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_2_04:  SM1IMM(__JudySearchLeaf2,  4);
	case cJ1_JPIMMED_2_05:  SM1IMM(__JudySearchLeaf2,  5);
	case cJ1_JPIMMED_2_06:  SM1IMM(__JudySearchLeaf2,  6);
	case cJ1_JPIMMED_2_07:  SM1IMM(__JudySearchLeaf2,  7);
#endif

#if (JUDY1 || JU_64BIT)
	case cJU_JPIMMED_3_02:  SM1IMM(__JudySearchLeaf3,  2);
#endif
#if (JUDY1 && JU_64BIT)
	case cJ1_JPIMMED_3_03:  SM1IMM(__JudySearchLeaf3,  3);
	case cJ1_JPIMMED_3_04:  SM1IMM(__JudySearchLeaf3,  4);
	case cJ1_JPIMMED_3_05:  SM1IMM(__JudySearchLeaf3,  5);

	case cJ1_JPIMMED_4_02:  SM1IMM(__JudySearchLeaf4,  2);
	case cJ1_JPIMMED_4_03:  SM1IMM(__JudySearchLeaf4,  3);

	case cJ1_JPIMMED_5_02:  SM1IMM(__JudySearchLeaf5,  2);
	case cJ1_JPIMMED_5_03:  SM1IMM(__JudySearchLeaf5,  3);

	case cJ1_JPIMMED_6_02:  SM1IMM(__JudySearchLeaf6,  2);

	case cJ1_JPIMMED_7_02:  SM1IMM(__JudySearchLeaf7,  2);
#endif
...
//
// Note:  If a found JP contains needed Dcd bytes, that's OK, they're copied to
...
// If there's a next-left/right JP in the current BranchB, which for
...
//
// Note:  Certain JP types (cJU_JAPLEAF, cJL_JAPLEAF_POPU1, cJL_JAPLEAF_POPU2)
// are not expected here and fall into the default case.
//
...
SM3BranchB:
	    ...
	    while (! (JU_JBB_BITMAP(Pjbb, ++subexp)))  // find non-empty subexp.
	    {
		if (subexp >= cJU_NUMSUBEXPB - 1)      // didn't find one.
		{
		    JU_SET_ERRNO(PJError, JU_ERRNO_CORRUPT);
		    JUDY1CODE(return(JERRI );)
		    JUDYLCODE(return(PPJERR);)
...
#else
#define	SM3LEAFL_SETPOP1  pop1 = JU_JPLEAF_POP0(Pjp->jp_DcdPop0) + 1
#endif
...
#define	SM3LEAFLNODCD			\
	Pjll = P_JLL(Pjp->jp_Addr);	\
	SM3LEAFL_SETPOP1;		\
	offset = JU_JPLEAF_POP0(Pjp->jp_DcdPop0); assert(offset >= 0)
#else
#define	SM3LEAFLNODCD			\
	Pjll = P_JLL(Pjp->jp_Addr);	\
	SM3LEAFL_SETPOP1;		\
	offset = 0; assert(JU_JPLEAF_POP0(Pjp->jp_DcdPop0) >= 0);
#endif
...
// the digit for the right-most Index with a bit set?
	    ...
	    while (! JU_JLB_BITMAP(Pjlb, ++subexp))  // find non-empty subexp.
	    {
		if (subexp >= cJU_NUMSUBEXPL - 1)    // didn't find one.
		{
		    JU_SET_ERRNO(PJError, JU_ERRNO_CORRUPT);
		    JUDY1CODE(return(JERRI );)
		    JUDYLCODE(return(PPJERR);)
