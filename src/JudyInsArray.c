...
static uint8_t _branchL_JPtype[] = {
    0,
    0,
    cJU_JPBRANCH_L2,
    cJU_JPBRANCH_L3,
...
static uint8_t _branchB_JPtype[] = {
    0,
    0,
    cJU_JPBRANCH_B2,
    cJU_JPBRANCH_B3,
...
static uint8_t _branchU_JPtype[] = {
    0,
    0,
    cJU_JPBRANCH_U2,
    cJU_JPBRANCH_U3,
...
// the first digit's bits.  Avoid doing variable shifts by precomputing a
// lookup array.

static Word_t _subexp_mask[] = {
    0,
    ~cJU_POP0MASK(1),
    ~cJU_POP0MASK(2),
    ~cJU_POP0MASK(3),
...
static bool_t __JudyInsArray(Pjp_t PjpParent, int Level, PWord_t PPop1,
			     PWord_t PIndex,
#ifdef JUDYL
			     Pjv_t   PValue,
#endif
...
//
// TBD:  Until that's written, note that the function returns 1 for success or
...
// Common code for unusual error handling when no JPM available:
	...
	if (Count > cJU_JAPLEAF_MAXPOP1)	// too big for root-level leaf.
	{
...
// 0, in which case free the JPM itself:
	    ...
	    if (! __JudyInsArray(&(Pjpm->jpm_JP), cJU_ROOTSTATE, &Count,
				 (PWord_t) PIndex,
#ifdef JUDYL
				 (Pjv_t) PValue,
#endif
...
#if (LOW_POP && JUDYL)
	if (Count == 1)				// special root-level leaf:
	{
	    Pjlw      = __JudyAllocJLW(1);
			JU_CHECKALLOC(Pjlw_t, Pjlw, JERRI);
	    *PPArray  = (Pvoid_t) ((Word_t) Pjlw | cJL_JAPLEAF_POPU1);
	    Pjlwindex = Pjlw;
	}
	else
	if (Count == 2)				// special root-level leaf:
	{
	    Pjlw      = __JudyAllocJLW(2);
			JU_CHECKALLOC(Pjlw_t, Pjlw, JERRI);
	    *PPArray  = (Pvoid_t) ((Word_t) Pjlw | cJL_JAPLEAF_POPU2);
	    Pjlwindex = Pjlw;
	}
	else					// generic root-level leaf:
#endif // (LOW_POP && JUDYL)
	{
	    Pjlw      = __JudyAllocJLW(Count + 1);
			JU_CHECKALLOC(Pjlw_t, Pjlw, JERRI);
	    *PPArray  = (Pvoid_t) ((Word_t) Pjlw | cJU_JAPLEAF);
...
// - a pointer to a JP
//
// - the JP's level in the tree, that is, the number of digits left to decode
...
FUNCTION static bool_t __JudyInsArray(
	...
	Pjpm_t	Pjpm)			// for memory and errors.
{
	...
	int	levelsub;		// actual, of Pjp's node, <= Level.
...
// inclusive, to _Level, exclusive.  But in practice, since the indexes are all
// known to be identical above Level, it just uses a mask for the digits
// through _Level + 1; see _subexp_mask[].

#define	SAMESUBEXP(_Level) \
	(! ((PIndex[0] ^ PIndex[pop1 - 1]) & _subexp_mask[_Level]))
...
//
// Note:  This can set the JPM's JP to an invalid jp_Type, but it doesn't
// matter because the JPM is deleted by the caller.

#define	SETJPNULL_PARENT \
	{							\
	    (PjpParent->jp_Addr)    = 0;			\
	    (PjpParent->jp_DcdPop0) = 0;			\
	    (PjpParent->jp_Type)    = cJU_JPNULL1 + Level - 1;	\
	}
...
// caller's population count, and return FALSE:
//
// Note:  At Level == cJU_ROOTSTATE this sets the JPM's JP's jp_Type to a bogus
// value, but it doesn't matter because the JPM should be deleted by the
// caller.
...
#define	COPYTOLEAF_EVEN_SUB(Pjll,LeafType)		\
	{						\
	    LeafType * _Pleaf  = (LeafType *) (Pjll);	\
	    Word_t     _pop1   = pop1;			\
	    PWord_t    _PIndex = PIndex;		\
							\
	    assert(pop1 > 0);				\
							\
	    do { *_Pleaf++ = *_PIndex++; /* truncates */\
	    } while (--(_pop1));			\
	}

#define	COPYTOLEAF_ODD_SUB(cLevel,Pjll,Copy)		\
	{						\
	    uint8_t * _Pleaf  = (uint8_t *) (Pjll);	\
	    Word_t    _pop1   = pop1;			\
	    PWord_t   _PIndex = PIndex;			\
							\
	    assert(pop1 > 0);				\
...
#define	MAKELEAF_SUB2(cLevel,JPType)					\
	...
	(PjpParent->jp_Addr)	= (Word_t) PjllRaw;			\
	(PjpParent->jp_DcdPop0) = (*PIndex & cJU_DCDMASK(cLevel)) | (pop1 - 1);\
...
// ------ START OF CODE ------
	...
	assert((Level <  cJU_ROOTSTATE) || (pop1 > cJU_JAPLEAF_MAXPOP1));


// CHECK FOR TOP LEVEL:
//
// Special case:  If at the top level (PjpParent is in the JPM), a top-level
// branch must be created, even if it's a BranchL with just one JP.  (The JPM
...
// would suffice), and the JPM's JP can't handle a narrow pointer because the
// jp_DcdPop0 field isn't big enough.)  Otherwise continue to check for a pop1
...
// includes a pointer to a values-only leaf.
	...
	if (pop1 <= _immed_maxpop1[Level])	// note: always < root level.
	{
...
#ifdef JUDYL
	    if ((PjvRaw = __JudyLAllocJV(pop1, Pjpm)) == (Pjv_t) NULL)
		NOMEM;
...
// capacity, see later), check if pop1 fits in a leaf (using _leaf_maxpop1[])
...
// lower level:
	    ...
	    if (pop1 > _leaf_maxpop1[levelsub]) continue;
...
// lower level (levelsub):
		...
		goto BuildBranch;	// can't use a narrow, need a branch.
...
//
// Note:  The jp_DcdPop0 and jp_Type assignments in MAKELEAF_* happen correctly
// for the levelsub (not Level) of the new leaf, even if it's under a narrow
// pointer.

	    switch (levelsub)
	    {
#if (JUDYL || (! JU_64BIT))
	    case 1: MAKELEAF_EVEN(1, cJU_JPLEAF1, __JudyAllocJLL1,
				  JL_LEAF1VALUEAREA, uint8_t);
		    break;
#endif
	    case 2: MAKELEAF_EVEN(2, cJU_JPLEAF2, __JudyAllocJLL2,
				  JL_LEAF2VALUEAREA, uint16_t);
		    break;
	    case 3: MAKELEAF_ODD( 3, cJU_JPLEAF3, __JudyAllocJLL3,
				  JL_LEAF3VALUEAREA, JU_COPY3_LONG_TO_PINDEX);
		    break;
#ifdef JU_64BIT
	    case 4: MAKELEAF_EVEN(4, cJU_JPLEAF4, __JudyAllocJLL4,
				  JL_LEAF4VALUEAREA, uint32_t);
		    break;
	    case 5: MAKELEAF_ODD( 5, cJU_JPLEAF5, __JudyAllocJLL5,
				  JL_LEAF5VALUEAREA, JU_COPY5_LONG_TO_PINDEX);
		    break;
	    case 6: MAKELEAF_ODD( 6, cJU_JPLEAF6, __JudyAllocJLL6,
				  JL_LEAF6VALUEAREA, JU_COPY6_LONG_TO_PINDEX);
		    break;
	    case 7: MAKELEAF_ODD( 7, cJU_JPLEAF7, __JudyAllocJLL7,
...
//
// See above about JPLEAF*.  If pop1 doesn't fit in any level of linear leaf,
...
// JPFULLPOPU1:
	    ...
	    {
		(PjpParent->jp_DcdPop0)	= (*PIndex & cJU_DCDMASK(1))
					| cJU_JPFULLPOPU1_POP0;
		(PjpParent->jp_Type)	= cJ1_JPFULLPOPU1;
		return(TRUE);
	    }
#endif
...
// Build subexpanse values-only leaves (LeafVs) under LeafB1:
	    ...
	    {
		if (! (pop1sub = __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, offset))))
...
// subexpanses and adjust *PPop1:
		...
		if ((PjvRaw = __JudyLAllocJV(pop1sub, Pjpm))
		 == (Pjv_t) NULL)
		{
		    for (/* null */; offset < cJU_NUMSUBEXPL; ++offset)
		    {
			*PPop1 -= __JudyCountBitsL(JU_JLB_BITMAP(Pjlb, offset));
...
// Attach new LeafB1 to parent JP; note use of *PPop1 possibly < pop1:
	    ...
	    (PjpParent->jp_Addr)    = (Word_t) PjlbRaw;
	    (PjpParent->jp_DcdPop0) = (*PIndex & cJU_DCDMASK(1)) | (*PPop1 - 1);
	    (PjpParent->jp_Type)    = cJU_JPLEAF_B1;
...
BuildBranch:	// come here directly if a leaf won't work.
...
// which a branch can be used.  There's no need to check SAMESUBEXP at level 2
// because it's known to be false at level 2-1 = 1.
//
...
// (number of JPs) needed.
	...
	if ((PjbuRaw = __JudyAllocJBU(Pjpm)) == (Pjbu_t) NULL) NOMEM;
	Pjbu = P_JBU(PjbuRaw);

	JPtype_null	  = cJU_JPNULL1 + levelsub - 2;  // in new BranchU.
	JPnull.jp_Addr	  = 0;
	JPnull.jp_DcdPop0 = 0;
	JPnull.jp_Type	  = JPtype_null;
...
// Count indexes in digit's subexpanse:
...
// First shortcut by handling pop1sub == 1 (JPIMMED_*_01) inline locally.
	    ...
	    {
      JUDYLCODE((Pjp->jp_Addr)    = (Word_t) (*PValue++);)
		(Pjp->jp_DcdPop0) = *PIndex;		// truncates.
		(Pjp->jp_Type)    = cJU_JPIMMED_1_01 + levelsub - 2;
...
// Recurse to populate one digit's (subexpanse's) JP; if successful, skip
// indexes (and values) just stored (performance path), except when expanse is
// completely stored:

	    DBGCODE(pop1subprev = pop1sub;)

	    if (__JudyInsArray(Pjp, levelsub - 1, &pop1sub, (PWord_t) PIndex,
#ifdef JUDYL
			       (Pjv_t) PValue,
#endif
			       Pjpm))
	  ...
	  JUDYLCODE(PValue += pop1sub;)
		...
		// else leave PIndex in BranchU's expanse.

// No more indexes to store in BranchU's expanse:
...
// passed to __JudyInsArray(); skip this JP later during ClearBranch.

	    assert(pop1subprev > pop1sub);	// check __JudyInsArray().
	    assert(pop1	       > pop1sub);	// check __JudyInsArray().
...
// indexes to store in the BranchU's expanse:
//
...
// COMPRESS JPBRANCH_U* TO JPBRANCH_L*:
	...
	if (numJPs <= cJU_BRANCHLMAXJPS)	// JPs fit in a BranchL.
	{
	    ...
	    if ((*PPop1 > JU_BRANCHL_MAX_POP)	// pop too high.
	     || ((PjblRaw = __JudyAllocJBL(Pjpm)) == (Pjbl_t) NULL))
	    {					// can't alloc BranchL.
...
// Free the BranchU and prepare to use the new BranchL instead:
	    ...
	    JPtype = _branchL_JPtype[levelsub];
...
// the opposite of how it's used in single-insert code.
	...
	else
	{
	    ...
	    if ((*PPop1 > JU_BRANCHB_MAX_POP)	// pop too high.
	     || ((PjbbRaw = __JudyAllocJBB(Pjpm)) == (Pjbb_t) NULL))
	    {					// can't alloc BranchB.
		goto SetParent;			// just keep BranchU.
	    }
...
// continue to use the BranchU:
		    ...
		    == (Pjp_t) NULL)
		{
			...
			__JudyFreeJBBJP(JU_JBB_PJP(Pjbb, offset),
				 __JudyCountBitsB(JU_JBB_BITMAP(Pjbb, offset)),
					Pjpm);
		    }
...
// Set one JP subarray pointer and copy the subexpanse's JPs to the subarray:
//
...
// Free the BranchU and prepare to use the new BranchB instead:
	    ...
	    JPtype = _branchB_JPtype[levelsub];
...
SetParent:
	...
	{
	    (PjpParent->jp_DcdPop0) = (*PIndex & cJU_DCDMASK(levelsub))
				    | (*PPop1 - 1);
	}
